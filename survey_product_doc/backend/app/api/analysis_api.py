...
from backend.app.services.chart_service import get_question_option_stats_by_department
...
def get_survey_analysis_stats(
    ...
    if dimension not in ["department", "position"]:
        raise HTTPException(
            ...
            detail="Invalid dimension. Must be 'department' or 'position'."
        )
    stats = get_survey_stats_by_dimension(db, survey_id, dimension)
    return {
        "dimension": dimension,
        "stats": stats
    }
...
def get_question_scores(
    ...
    """
    scores = get_per_question_scores(db, survey_id, department=department, position=position)
    return scores
...
def get_survey_option_charts(
    ...
    获取选项分布图表数据（包含部门构成）
    """
    charts = get_question_option_stats_by_department(db, survey_id)
    return charts
